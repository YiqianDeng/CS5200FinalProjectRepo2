...
const ReserveServicesForReservationComponent = () => {
    const history = useHistory()
    const [reserveServices, setReserveServices] = useState([])
    const {reservationId} = useParams()

    useEffect(() => {
        findReserveServicesForReservation(reservationId)
    }, [])

    const findReserveServicesForReservation = (reservationId) =>
        reserveServiceService.findReserveServicesForReservation(reservationId)
            .then(reserveServices => setReserveServices(reserveServices))

    return(
        <div className="container">
            <h2>Reserve Service List of Reservation {reservationId}</h2>
...
export default ReserveServicesForReservationComponent;
